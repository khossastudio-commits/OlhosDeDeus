  ...
  try {
    ...
    const events = (feed.features ?? []).filter((f: any) => {
      ...
      return {
        ...
        source: source('usgs-earthquakes', 'USGS Earthquakes', 'https://earthquake.usgs.gov/earthquakes/feed/'),
        tags: ['USGS', 'GeoJSON'],
      };
    });
  ...
  try {
    ...
    const events = rows.map((row: Record<string, string>, index) => {
      ...
      return {
        ...
        source: source('nasa-firms', 'NASA FIRMS', 'https://firms.modaps.eosdis.nasa.gov/', { licenseOrTerms: 'NASA FIRMS terms' }), tags: ['NASA', 'FIRMS', 'VIIRS'],
      };
...
export async function getAviation(): Promise<FeedResult> {
  try {
    ...
    url.searchParams.set('lamin', String(B.south)); url.searchParams.set('lomin', String(B.west));
    url.searchParams.set('lamax', String(B.north)); url.searchParams.set('lomax', String(B.east));
    ...
    if (!response.ok) return { events: [], source: null, status: 'unavailable' };
    const data = await response.json();
    const events = (data.states ?? []).filter((s: any[]) => Number.isFinite(s[5]) && Number.isFinite(s[6])).map((s: any[]) => ({
      id: `opensky-${s[0]}`, layer: 'aviation', title: String(s[1] ?? s[0] ?? 'Aeronave').trim() || 'Aeronave',
      summary: `Alt ${Math.round(Number(s[13] ?? 0))} m · ${Math.round(Number(s[9] ?? 0) * 3.6)} km/h`, severity: 'info', observedAt: new Date((Number(data.time ?? Date.now() / 1000)) * 1000).toISOString(),
      location: { latitude: Number(s[6]), longitude: Number(s[5]) }, confidence: 1,
      source: source('opensky', 'OpenSky Network', 'https://opensky-network.org/data/api/'), tags: ['aviation', 'ADS-B'],
      metadata: { icao24: s[0], callsign: s[1], velocityMs: s[9], heading: s[10], altitudeM: s[13] },
    }));
...
export async function getDisasters(): Promise<FeedResult> {
  try {
    ...
    url.searchParams.set('eventlist', 'EQ;TC;FL;WF');
    url.searchParams.set('fromdate', new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10));
    ...
    const events = features.map((f: any) => {
      ...
      return { f, longitude, latitude, p };
    }).filter((x: any) => inside(x.latitude, x.longitude)).map((x: any) => ({
      id: `gdacs-${x.p.eventid ?? x.p.eventId ?? Math.random().toString(36).slice(2)}`, layer: 'disasters',
      title: String(x.p.name ?? x.p.eventname ?? x.p.eventtype ?? 'Alerta de desastre'), summary: `GDACS · ${String(x.p.alertlevel ?? x.p.alertLevel ?? 'sem nível')}`,
      severity: String(x.p.alertlevel ?? x.p.alertLevel ?? '').toLowerCase() === 'red' ? 'critical' : String(x.p.alertlevel ?? '').toLowerCase() === 'orange' ? 'warning' : 'watch',
      observedAt: new Date(x.p.fromdate ?? x.p.fromDate ?? Date.now()).toISOString(), location: { latitude: x.latitude, longitude: x.longitude }, confidence: 1,
      source: source('gdacs', 'GDACS', 'https://www.gdacs.org/', { licenseOrTerms: 'GDACS terms of use' }), tags: ['GDACS', 'disaster'],
    }));
...
export async function getNews(): Promise<FeedResult> {
  try {
    ...
    url.searchParams.set('query', '(Mozambique OR Moçambique)'); url.searchParams.set('mode', 'artlist'); url.searchParams.set('format', 'json');
    url.searchParams.set('maxrecords', '25'); url.searchParams.set('timespan', '24h'); url.searchParams.set('sort', 'datedesc');
    const response = await fetch(url, { next: { revalidate: 300 } });
    if (!response.ok) return { events: [], source: null, status: 'unavailable' };
    const data = await response.json();
    const events = (data.articles ?? []).map((a: any, index: number) => ({
      id: `gdelt-${a.url ?? index}`, layer: 'news', title: String(a.title ?? 'Notícia sobre Moçambique'), summary: String(a.domain ?? a.sourcecountry ?? 'Fonte pública'),
      severity: 'info', observedAt: a.seendate ? `${String(a.seendate).slice(0, 4)}-${String(a.seendate).slice(4, 6)}-${String(a.seendate).slice(6, 8)}T${String(a.seendate).slice(9, 11) || '00'}:${String(a.seendate).slice(11, 13) || '00'}:00Z` : now(),
...
export function getOptionalFeedStatus() {
  return {
    maritime: process.env.AISSTREAM_API_KEY ? 'configured' : 'not_configured',
    satellites: process.env.N2YO_API_KEY ? 'configured' : 'not_configured',
    cameras: process.env.PUBLIC_CAMERA_FEED_URL ? 'configured' : 'authorized_feed_required',
  };
}

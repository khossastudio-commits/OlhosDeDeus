...
type IntelEvent = {
  id: string; layer: OlhosLayerId; title: string; summary?: string; severity: string; observedAt: string; confidence?: number;
...
export default function OlhosCommandCenter() {
  ...
  const mapData = useMemo(() => {
    ...
    const earthquakes = events.filter((e) => e.layer === 'earthquakes' && e.location).map((e) => ({ id: e.id, lat: e.location!.latitude, lng: e.location!.longitude, magnitude: Number(e.title.match(/M([0-9.]+)/i)?.[1] ?? 2.5), place: e.summary ?? 'Evento sísmico', depth: 0, source: e.source?.name ?? 'USGS' }));
